...
using var host = Host.CreateDefaultBuilder()
            ...
            {
                services.AddOpenAIService(settings => { settings.ApiKey = Environment.GetEnvironmentVariable("OPENAI_API_KEY") ?? throw new InvalidOperationException("Set OPENAI_API_KEY."); });
                //services.AddOpenAIServices(options => { options.ApiKey = Environment.GetEnvironmentVariable("OPENAI_API_KEY"); });
            })
            .Build();
